...
<div class="max-w-lg">
    <div class="rounded-2xl border border-black/5 bg-white p-6">
        <form method="POST" action="{{ route('admin.legalitas.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf

            <div>
                <label for="nama" class="block text-sm font-medium text-ink/70 mb-1.5">Nama</label>
                <input id="nama" name="nama" type="text" value="{{ old('nama') }}" autofocus
            ...
            <div>
                <label for="icon" class="block text-sm font-medium text-ink/70 mb-1.5">Logo/Icon</label>
                ...
                <p class="mt-1.5 text-xs text-ink/40">Gambar (JPG, PNG, WEBP, SVG). Maksimal 2MB.</p>
            </div>
