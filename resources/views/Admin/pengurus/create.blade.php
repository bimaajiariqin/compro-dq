...
<div class="max-w-lg">
    <div class="rounded-2xl border border-black/5 bg-white p-6">
        <form method="POST" action="{{ route('admin.pengurus.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <select id="kelompok" name="kelompok"
                    ...
                    @foreach ($kelompokOptions as $opsi)
                        <option value="{{ $opsi }}" {{ old('kelompok') === $opsi ? 'selected' : '' }}>{{ $opsi }}</option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center gap-2">
                <input id="is_ketua" name="is_ketua" type="checkbox" value="1" {{ old('is_ketua') ? 'checked' : '' }}
            ...
            <div class="grid sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input id="urutan_grup" name="urutan_grup" type="number" value="{{ old('urutan_grup', 0) }}" min="0"
                           class="w-full rounded-xl border border-black/10 bg-white px-4 py-2.5 text-sm
                                  focus:outline-none focus:ring-2 focus:ring-emerald-700/30 focus:border-emerald-700">
            ...
            <div>
                <label for="foto" class="block text-sm font-medium text-ink/70 mb-1.5">Foto (opsional)</label>
                ...
                <p class="mt-1.5 text-xs text-ink/40">Kosongkan jika belum ada foto. Maksimal 2MB.</p>
            </div>
